...
schema
    ...
    .has().digits(1)
    .has().not().spaces()
    .is().not().oneOf(['Passw0rd', 'Password123', 'Admin@1234', 'Qwerty@123'])
...
async function createUser(req, res) {
    if (schema.validate(req.body.password)) {
        ...
        res.send({ result: "Fail", message: "Password Length Must Be Greater or Equal to 8 and Less Than or Equa to 100, It Must Contains Atleast 1 Digits,1 Upper Case Character and It Can't Contain any Space " })
}
    ...
    try {
        ...
        if (data) {
            ...
            const mailOptions = {
                ...
                to: data.email,
                subject: "OTP For Password Reset!!! : BestDeals",
                text: `
...
async function forgetPassword2(req, res) {
    try {
        ...
        if (data) {
            ...
            res.send({ result: "Fail", message: "UnAuthorized Acitivity" })
